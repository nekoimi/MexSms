...
class AliyunGateway extends AbstractGateway {
    ...
    public function send(string $toPhoneNumber, MessageInterface $message, Config $config) {
        ...
        try {
            ...
            if ('OK' != $result['Code']) {
                $this->log()->error(__CLASS__ . " : 短信发送失败! [ $toPhoneNumber ]  ===> Code : {$result['Code']}  Des: [ {$result['Message']} ] ");
                return false;
            }
    ...
    public function verify(string $phoneNumber, $smsCode, Config $config): bool {
        $verifyCls = $config->get($this->getName() . '.verify_handler');
        if (is_null($verifyCls) || !class_exists($verifyCls)) {
            throw new VerifyHandlerNotExistsException();
        }
        $verifyHandler = new $verifyCls;
